...
const SUBJECT_COLORS = ['bg-[#1c2d45] text-[#5b9cf5]', 'bg-[#1a3328] text-[#34d399]', 'bg-[#2a1e3a] text-[#9b82e8]', 'bg-[#332200] text-[#e5a731]', 'bg-[#351c2d] text-[#d96a9e]', 'bg-[#1a3232] text-[#3cbfcf]', 'bg-[#302e1a] text-[#d4b032]', 'bg-[#351c20] text-[#e8656d]']
...
export default function SettingsPage() {
      ...
      {[1, 2, 3, 4, 5, 6].map(d => (
        <button
          ...
          className={`w-7 h-7 rounded text-xs font-medium ${
            days.includes(d) ? 'bg-[#3182f6] text-white' : 'bg-[#36363e] text-[#8b8b9a] hover:bg-gray-200'
          }`}
  ...
  if (loading) return (
    <div className="animate-pulse">
      ...
      <div className="flex gap-2 mb-6">
        <div className="flex-1 h-10 bg-[#36363e] rounded-lg"></div>
        <div className="h-10 bg-gray-200 rounded-lg w-28"></div>
      </div>
      <div className="space-y-3">
        {Array.from({ length: 3 }, (_, gi) => (
          <div key={gi} className="bg-[#212126] rounded-xl border overflow-hidden">
  ...
  return (
    <div>
      ...
      {/* 학년별 반 관리 */}
      {grades.length === 0 ? (
        <div className="text-center py-12 text-[#5e5e6e]">반을 추가해주세요</div>
      ) : (
            ...
            return (
              <div key={grade.id} className="card overflow-hidden">
                <div className="flex items-center gap-2 px-4 py-3">
                  ...
                  <span className="text-xs text-[#5e5e6e] mr-1">{grade.classes.length}개 반</span>
                  <span className="text-xs text-[#5e5e6e]">{totalStudents}명</span>
                </div>

                <div className="border-t border-[#2c2c33] bg-[#2c2c33]/50 px-4 py-3">
                  {grade.classes.length > 0 && (
                    <div className="space-y-2 mb-3">
                      {grade.classes.map((cls, clsIdx) => (
                        <div key={cls.id} className="flex items-center gap-1.5 sm:gap-2 bg-[#212126] rounded-lg px-2 sm:px-3 py-2">
                          {editingClassId === cls.id ? (
                            <div className="flex-1 space-y-2">
                              <div className="flex items-center gap-2">
                                ...
                                <button onClick={() => setEditingClassId(null)} className="text-[#5e5e6e]" aria-label="취소"><X className="w-4 h-4" /></button>
                              </div>
                              <div className="flex items-center gap-2 pl-1">
                                <span className="text-xs text-[#8b8b9a]">수업 요일</span>
                                <DayPicker days={editClassDays} setDays={setEditClassDays} />
                              </div>
                              {teachers.length > 0 && (
                                <div className="flex items-center gap-2 pl-1">
                                  <span className="text-xs text-[#8b8b9a]">선생님</span>
                                  <select value={editClassTeacherId} onChange={e => setEditClassTeacherId(e.target.value)} className="px-2 py-1 border rounded text-sm focus:outline-none focus:ring-2 focus:ring-[#3182f6] bg-[#212126]">
                            ...
                            <>
                              ...
                              <div className="flex flex-col shrink-0">
                                <button
                                  ...
                                  disabled={clsIdx === 0}
                                  className="p-0 text-[#5e5e6e] hover:text-[#8b8b9a] disabled:opacity-20 disabled:hover:text-[#5e5e6e]"
                                  aria-label="위로"
                                >
                              ...
                              {getTeacherName(cls.teacher_id) && (
                                <span className="text-[10px] sm:text-xs text-[#8b8b9a] shrink-0 hidden sm:inline">{getTeacherName(cls.teacher_id)}</span>
                              )}
                              {cls.class_days && (
                                <span className="text-[10px] sm:text-xs text-[#5e5e6e] shrink-0 hidden sm:inline">{parseClassDays(cls.class_days)?.map(d => DAY_LABELS[d]).join('/')}</span>
                              )}
                              <span className="text-xs sm:text-sm font-medium text-[#3182f6] shrink-0">{formatFee(cls.monthly_fee)}</span>
                              <span className="text-[10px] sm:text-xs text-[#5e5e6e] shrink-0">{getActiveStudents(cls.students ?? []).length}명</span>
                              <button onClick={() => openTransfer(cls)} className="p-0.5 sm:p-1 text-[#5e5e6e] hover:text-[#3182f6] shrink-0" aria-label="학생 반이동" title="학생 반이동">
                                <ArrowRightLeft className="w-3 h-3 sm:w-3.5 sm:h-3.5" />
                              </button>
                              <button onClick={() => { setEditingClassId(cls.id); setEditClassName(cls.name); setEditClassFee(String(cls.monthly_fee)); setEditClassSubject(cls.subject || ''); setEditClassDays(parseClassDays(cls.class_days) ?? []); setEditClassTeacherId(cls.teacher_id || '') }} className="p-0.5 sm:p-1 text-[#5e5e6e] hover:text-[#8b8b9a] shrink-0" aria-label="반 수정">
                                <Pencil className="w-3 h-3 sm:w-3.5 sm:h-3.5" />
                              </button>
                              <button onClick={() => deleteClass(cls.id, cls.name)} className="p-0.5 sm:p-1 text-[#5e5e6e] hover:text-red-500 shrink-0" aria-label="반 삭제">
                  ...
                  {addingClassToGrade === grade.id ? (
                    <div className="space-y-2">
                      <div className="flex items-center gap-2">
                        ...
                        <button onClick={() => { setAddingClassToGrade(null); resetClassForm() }} className="text-[#5e5e6e]" aria-label="취소"><X className="w-4 h-4" /></button>
                      </div>
                      <div className="flex items-center gap-2 pl-1">
                        <span className="text-xs text-[#8b8b9a]">수업 요일</span>
                      ...
                      {teachers.length > 0 && (
                        <div className="flex items-center gap-2 pl-1">
                          <span className="text-xs text-[#8b8b9a]">선생님</span>
                          <select value={newClassTeacherId} onChange={e => setNewClassTeacherId(e.target.value)} className="px-2 py-1.5 border rounded text-sm focus:outline-none focus:ring-2 focus:ring-[#3182f6] bg-[#212126]">
                            <option value="">없음</option>
                            {teachers.map(t => <option key={t.id} value={t.id}>{t.name}{t.subject ? ` (${t.subject})` : ''}</option>)}
                          </select>
                        </div>
      ...
      {/* 선생님 관리 */}
      <div className="mt-8">
        <div className="flex items-center gap-2 text-[15px] font-bold text-[#c0c0cc] mb-3">
            ...
            <div className="px-4 py-3">
              {teachers.length > 0 && (
                <div className="space-y-2 mb-3">
                  {teachers.map((teacher, tIdx) => (
                    <div key={teacher.id} className="flex items-center gap-2 bg-[#2c2c33] rounded-lg px-3 py-2">
                      {editingTeacherId === teacher.id ? (
                        <div className="flex-1 flex items-center gap-2">
                          ...
                          <button onClick={() => setEditingTeacherId(null)} className="text-[#5e5e6e] shrink-0" aria-label="취소"><X className="w-4 h-4" /></button>
                        </div>
                      ) : (
                        <>
                          <div className="flex flex-col shrink-0">
                            <button onClick={() => swapTeacherOrder(tIdx, -1)} disabled={tIdx === 0} className="p-0 text-[#5e5e6e] hover:text-[#8b8b9a] disabled:opacity-20" aria-label="위로">
                              <ChevronUp className="w-3.5 h-3.5" />
                            </button>
                            <button onClick={() => swapTeacherOrder(tIdx, 1)} disabled={tIdx === teachers.length - 1} className="p-0 text-[#5e5e6e] hover:text-[#8b8b9a] disabled:opacity-20" aria-label="아래로">
                              <ChevronDown className="w-3.5 h-3.5" />
                            </button>
                          </div>
                          <span className="text-sm font-medium">{teacher.name}</span>
                          {teacher.subject && <span className="text-xs text-[#5e5e6e]">{teacher.subject}</span>}
                          {teacher.phone && <span className="text-xs text-[#5e5e6e]">{teacher.phone}</span>}
                          <span className="flex-1" />
                          <Link href={`/teachers/${teacher.id}`} className="p-1 text-[#5e5e6e] hover:text-[#3182f6]" aria-label="급여명세서" title="급여명세서">
                            <FileText className="w-3.5 h-3.5" />
                          </Link>
                          <button onClick={() => { setEditingTeacherId(teacher.id); setEditTeacherName(teacher.name); setEditTeacherPhone(teacher.phone || ''); setEditTeacherSubject(teacher.subject || '') }} className="p-1 text-[#5e5e6e] hover:text-[#8b8b9a]" aria-label="수정">
                            <Pencil className="w-3.5 h-3.5" />
                          </button>
                          <button onClick={() => deleteTeacher(teacher.id, teacher.name)} className="p-1 text-[#5e5e6e] hover:text-red-500" aria-label="삭제">
              ...
              {addingTeacher ? (
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={() => { setAddingTeacher(false); setNewTeacherName(''); setNewTeacherPhone(''); setNewTeacherSubject('') }} className="text-[#5e5e6e] shrink-0" aria-label="취소"><X className="w-4 h-4" /></button>
                </div>
              ) : (
                <button
      ...
      {/* 로그 & 로그아웃 */}
      <div className="mt-12 pt-6 border-t border-[#2c2c33] space-y-3">
        <button
          onClick={loadLogs}
          className="w-full py-3 text-[#8b8b9a] card text-sm font-medium hover:bg-[#2c2c33] flex items-center justify-center gap-2 transition-colors"
        >
        ...
        <button
          ...
          className="w-full py-3 text-[#e8656d] card text-sm font-medium hover:bg-[#351c20] flex items-center justify-center gap-2 transition-colors"
        >
      ...
      {transferClass && (
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center" onClick={() => setTransferClass(null)}>
          <div className="bg-[#212126] w-full sm:max-w-md sm:rounded-xl rounded-t-xl max-h-[80vh] flex flex-col" onClick={e => e.stopPropagation()}>
            <div className="px-5 py-4 border-b flex items-center justify-between shrink-0">
              <div>
                <h2 className="font-bold text-sm">학생 반이동</h2>
                <p className="text-xs text-[#5e5e6e] mt-0.5">{transferClass.name}</p>
              </div>
              <button onClick={() => setTransferClass(null)} className="p-1 text-[#5e5e6e] hover:text-[#8b8b9a]" aria-label="닫기">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto px-5 py-3">
              ...
              {(() => {
                ...
                if (activeStudents.length === 0) return (
                  <p className="text-sm text-[#5e5e6e] text-center py-6">이 반에 학생이 없습니다</p>
                )
                return (
                  <>
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-xs text-[#8b8b9a]">{selectedStudents.size}명 선택</span>
                      ...
                      {activeStudents.map(s => (
                        <button
                          ...
                          className={`w-full flex items-center gap-2 px-3 py-2.5 rounded-lg text-sm text-left transition-colors ${
                            selectedStudents.has(s.id) ? 'bg-[#3182f6]/10 text-[#3182f6]' : 'hover:bg-[#2c2c33]'
                          }`}
                        >
              ...
              {selectedStudents.size > 0 && (
                <div>
                  <label className="text-xs text-[#8b8b9a] mb-1.5 block font-medium">이동할 반</label>
                  <select
                    value={targetClassId}
                    onChange={e => setTargetClassId(e.target.value)}
                    className="w-full px-3 py-2.5 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#3182f6] bg-[#212126]"
                  >
      ...
      {showLogs && (
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center" onClick={() => setShowLogs(false)}>
          <div className="bg-[#212126] w-full sm:max-w-lg sm:rounded-xl rounded-t-xl max-h-[85vh] flex flex-col" onClick={e => e.stopPropagation()}>
            <div className="px-5 py-4 border-b flex items-center justify-between shrink-0">
              <h2 className="font-bold text-sm">변경 로그</h2>
              <button onClick={() => setShowLogs(false)} className="p-1 text-[#5e5e6e] hover:text-[#8b8b9a]" aria-label="닫기">
                <X className="w-5 h-5" />
              </button>
            </div>
            <div className="flex-1 overflow-y-auto px-5 py-3">
              {logsLoading ? (
                <div className="text-center py-12 text-[#5e5e6e] text-sm">불러오는 중...</div>
              ) : logs.length === 0 ? (
                <div className="text-center py-12 text-[#5e5e6e] text-sm">로그가 없습니다</div>
              ) : (
                <div className="space-y-2">
                  {logs.map(log => {
                    const actionColor = log.action === 'create' ? 'text-[#34d399] bg-[#1a3328]' : log.action === 'delete' ? 'text-[#e8656d] bg-[#351c20]' : 'text-[#5b9cf5] bg-[#1c2d45]'
                    const actionLabel = log.action === 'create' ? '생성' : log.action === 'delete' ? '삭제' : '수정'
                    const date = new Date(log.created_at)
                    const timeStr = `${date.getMonth() + 1}/${date.getDate()} ${String(date.getHours()).padStart(2, '0')}:${String(date.getMinutes()).padStart(2, '0')}`
                    return (
                      <div key={log.id} className="flex items-start gap-2 py-2 border-b border-[#2c2c33] last:border-0">
                        <span className={`text-[10px] px-1.5 py-0.5 rounded font-medium shrink-0 mt-0.5 ${actionColor}`}>{actionLabel}</span>
                        <div className="flex-1 min-w-0">
                          <p className="text-sm text-[#ececec] break-words">{log.summary}</p>
                          <p className="text-[10px] text-[#5e5e6e] mt-0.5">{timeStr}</p>
                        </div>
                      </div>
                    )
                  })}
