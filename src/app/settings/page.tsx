...
export default function SettingsPage() {
      ...
      {[1, 2, 3, 4, 5, 6].map(d => (
        <button
          ...
          className={`w-7 h-7 rounded text-xs font-medium ${
            days.includes(d) ? 'bg-[var(--blue)] text-white' : 'bg-elevated text-secondary hover:bg-elevated'
          }`}
  ...
  if (loading) return (
    <div className="animate-pulse">
      ...
      <div className="flex gap-2 mb-6">
        <div className="flex-1 h-10 bg-elevated rounded-lg"></div>
        <div className="h-10 bg-gray-200 rounded-lg w-28"></div>
      </div>
      <div className="space-y-3">
        {Array.from({ length: 3 }, (_, gi) => (
          <div key={gi} className="bg-surface rounded-xl border overflow-hidden">
  ...
  return (
    <div>
      ...
      {/* 학년별 반 관리 */}
      {grades.length === 0 ? (
        <div className="text-center py-12 text-tertiary">반을 추가해주세요</div>
      ) : (
            ...
            return (
              <div key={grade.id} className="card overflow-hidden">
                <div className="flex items-center gap-2 px-4 py-3">
                  ...
                  <span className="text-xs text-tertiary mr-1">{grade.classes.length}개 반</span>
                  <span className="text-xs text-tertiary">{totalStudents}명</span>
                </div>

                <div className="border-t border-default bg-surface-hover/50 px-4 py-3">
                  {grade.classes.length > 0 && (
                    <div className="space-y-2 mb-3">
                      {grade.classes.map((cls, clsIdx) => (
                        <div key={cls.id} className="flex items-center gap-1.5 sm:gap-2 bg-surface rounded-lg px-2 sm:px-3 py-2">
                          {editingClassId === cls.id ? (
                            <div className="flex-1 space-y-2">
                              <div className="flex items-center gap-2">
                                ...
                                <span className="text-xs text-tertiary">원</span>
                                <button onClick={() => updateClass(cls.id)} className="text-green-600" aria-label="저장"><Check className="w-4 h-4" /></button>
                                <button onClick={() => setEditingClassId(null)} className="text-tertiary" aria-label="취소"><X className="w-4 h-4" /></button>
                              </div>
                              <div className="flex items-center gap-2 pl-1">
                                <span className="text-xs text-secondary">수업 요일</span>
                                <DayPicker days={editClassDays} setDays={setEditClassDays} />
                              </div>
                              {teachers.length > 0 && (
                                <div className="flex items-center gap-2 pl-1">
                                  <span className="text-xs text-secondary">선생님</span>
                                  <select value={editClassTeacherId} onChange={e => setEditClassTeacherId(e.target.value)} className="px-2 py-1 border rounded text-sm focus:outline-none focus:ring-2 focus:ring-[var(--blue)] bg-surface">
                            ...
                            <>
                              ...
                              <div className="flex flex-col shrink-0">
                                <button
                                  ...
                                  disabled={clsIdx === 0}
                                  className="p-0 text-tertiary hover:text-secondary disabled:opacity-20 disabled:hover:text-tertiary"
                                  aria-label="위로"
                                >
                              ...
                              {getTeacherName(cls.teacher_id) && (
                                <span className="text-[10px] sm:text-xs text-secondary shrink-0 hidden sm:inline">{getTeacherName(cls.teacher_id)}</span>
                              )}
                              {cls.class_days && (
                                <span className="text-[10px] sm:text-xs text-tertiary shrink-0 hidden sm:inline">{parseClassDays(cls.class_days)?.map(d => DAY_LABELS[d]).join('/')}</span>
                              )}
                              <span className="text-xs sm:text-sm font-medium text-brand shrink-0">{formatFee(cls.monthly_fee)}</span>
                              <span className="text-[10px] sm:text-xs text-tertiary shrink-0">{getActiveStudents(cls.students ?? []).length}명</span>
                              <button onClick={() => openTransfer(cls)} className="p-0.5 sm:p-1 text-tertiary hover:text-brand shrink-0" aria-label="학생 반이동" title="학생 반이동">
                                <ArrowRightLeft className="w-3 h-3 sm:w-3.5 sm:h-3.5" />
                              </button>
                              <button onClick={() => { setEditingClassId(cls.id); setEditClassName(cls.name); setEditClassFee(String(cls.monthly_fee)); setEditClassSubject(cls.subject || ''); setEditClassDays(parseClassDays(cls.class_days) ?? []); setEditClassTeacherId(cls.teacher_id || '') }} className="p-0.5 sm:p-1 text-tertiary hover:text-secondary shrink-0" aria-label="반 수정">
                                <Pencil className="w-3 h-3 sm:w-3.5 sm:h-3.5" />
                              </button>
                              <button onClick={() => deleteClass(cls.id, cls.name)} className="p-0.5 sm:p-1 text-tertiary hover:text-red-500 shrink-0" aria-label="반 삭제">
                  ...
                  {addingClassToGrade === grade.id ? (
                    <div className="space-y-2">
                      <div className="flex items-center gap-2">
                        ...
                        <button onClick={() => { setAddingClassToGrade(null); resetClassForm() }} className="text-tertiary" aria-label="취소"><X className="w-4 h-4" /></button>
                      </div>
                      <div className="flex items-center gap-2 pl-1">
                        <span className="text-xs text-secondary">수업 요일</span>
                      ...
                      {teachers.length > 0 && (
                        <div className="flex items-center gap-2 pl-1">
                          <span className="text-xs text-secondary">선생님</span>
                          <select value={newClassTeacherId} onChange={e => setNewClassTeacherId(e.target.value)} className="px-2 py-1.5 border rounded text-sm focus:outline-none focus:ring-2 focus:ring-[var(--blue)] bg-surface">
                            <option value="">없음</option>
                            {teachers.map(t => <option key={t.id} value={t.id}>{t.name}{t.subject ? ` (${t.subject})` : ''}</option>)}
                          </select>
                        </div>
                    ...
                    <button
                      ...
                      className="flex items-center gap-1 text-sm text-brand font-medium hover:opacity-70"
                    >
      ...
      {/* 선생님 관리 */}
      <div className="mt-8">
        <div className="flex items-center gap-2 text-[15px] font-bold text-label mb-3">
            ...
            <div className="px-4 py-3">
              {teachers.length > 0 && (
                <div className="space-y-2 mb-3">
                  {teachers.map((teacher, tIdx) => (
                    <div key={teacher.id} className="flex items-center gap-2 bg-surface-hover rounded-lg px-3 py-2">
                      {editingTeacherId === teacher.id ? (
                        <div className="flex-1 flex items-center gap-2">
                          ...
                          <button onClick={() => setEditingTeacherId(null)} className="text-tertiary shrink-0" aria-label="취소"><X className="w-4 h-4" /></button>
                        </div>
                      ) : (
                        <>
                          <div className="flex flex-col shrink-0">
                            <button onClick={() => swapTeacherOrder(tIdx, -1)} disabled={tIdx === 0} className="p-0 text-tertiary hover:text-secondary disabled:opacity-20" aria-label="위로">
                              <ChevronUp className="w-3.5 h-3.5" />
                            </button>
                            <button onClick={() => swapTeacherOrder(tIdx, 1)} disabled={tIdx === teachers.length - 1} className="p-0 text-tertiary hover:text-secondary disabled:opacity-20" aria-label="아래로">
                              <ChevronDown className="w-3.5 h-3.5" />
                            </button>
                          </div>
                          <span className="text-sm font-medium">{teacher.name}</span>
                          {teacher.subject && <span className="text-xs text-tertiary">{teacher.subject}</span>}
                          {teacher.phone && <span className="text-xs text-tertiary">{teacher.phone}</span>}
                          <span className="flex-1" />
                          <Link href={`/teachers/${teacher.id}`} className="p-1 text-tertiary hover:text-brand" aria-label="급여명세서" title="급여명세서">
                            <FileText className="w-3.5 h-3.5" />
                          </Link>
                          <button onClick={() => { setEditingTeacherId(teacher.id); setEditTeacherName(teacher.name); setEditTeacherPhone(teacher.phone || ''); setEditTeacherSubject(teacher.subject || '') }} className="p-1 text-tertiary hover:text-secondary" aria-label="수정">
                            <Pencil className="w-3.5 h-3.5" />
                          </button>
                          <button onClick={() => deleteTeacher(teacher.id, teacher.name)} className="p-1 text-tertiary hover:text-red-500" aria-label="삭제">
              ...
              {addingTeacher ? (
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={() => { setAddingTeacher(false); setNewTeacherName(''); setNewTeacherPhone(''); setNewTeacherSubject('') }} className="text-tertiary shrink-0" aria-label="취소"><X className="w-4 h-4" /></button>
                </div>
              ) : (
                <button
      ...
      {/* 로그 & 로그아웃 */}
      <div className="mt-12 pt-6 border-t border-default space-y-3">
        <button
          onClick={loadLogs}
          className="w-full py-3 text-secondary card text-sm font-medium hover:bg-surface-hover flex items-center justify-center gap-2 transition-colors"
        >
      ...
      {transferClass && (
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center" onClick={() => setTransferClass(null)}>
          <div className="bg-surface w-full sm:max-w-md sm:rounded-xl rounded-t-xl max-h-[80vh] flex flex-col" onClick={e => e.stopPropagation()}>
            <div className="px-5 py-4 border-b flex items-center justify-between shrink-0">
              <div>
                <h2 className="font-bold text-sm">학생 반이동</h2>
                <p className="text-xs text-tertiary mt-0.5">{transferClass.name}</p>
              </div>
              <button onClick={() => setTransferClass(null)} className="p-1 text-tertiary hover:text-secondary" aria-label="닫기">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto px-5 py-3">
              ...
              {(() => {
                ...
                if (activeStudents.length === 0) return (
                  <p className="text-sm text-tertiary text-center py-6">이 반에 학생이 없습니다</p>
                )
                return (
                  <>
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-xs text-secondary">{selectedStudents.size}명 선택</span>
                      <button onClick={selectAllStudents} className="text-xs text-brand font-medium">
                      ...
                      {activeStudents.map(s => (
                        <button
                          ...
                          className={`w-full flex items-center gap-2 px-3 py-2.5 rounded-lg text-sm text-left transition-colors ${
                            selectedStudents.has(s.id) ? 'bg-[var(--blue)]/10 text-brand' : 'hover:bg-surface-hover'
                          }`}
                        >
              ...
              {selectedStudents.size > 0 && (
                <div>
                  <label className="text-xs text-secondary mb-1.5 block font-medium">이동할 반</label>
                  <select
                    value={targetClassId}
                    onChange={e => setTargetClassId(e.target.value)}
                    className="w-full px-3 py-2.5 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[var(--blue)] bg-surface"
                  >
      ...
      {showLogs && (
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center" onClick={() => setShowLogs(false)}>
          <div className="bg-surface w-full sm:max-w-lg sm:rounded-xl rounded-t-xl max-h-[85vh] flex flex-col" onClick={e => e.stopPropagation()}>
            <div className="px-5 py-4 border-b flex items-center justify-between shrink-0">
              <h2 className="font-bold text-sm">변경 로그</h2>
              <button onClick={() => setShowLogs(false)} className="p-1 text-tertiary hover:text-secondary" aria-label="닫기">
                <X className="w-5 h-5" />
              </button>
            </div>
            <div className="flex-1 overflow-y-auto px-5 py-3">
              {logsLoading ? (
                <div className="text-center py-12 text-tertiary text-sm">불러오는 중...</div>
              ) : logs.length === 0 ? (
                <div className="text-center py-12 text-tertiary text-sm">로그가 없습니다</div>
              ) : (
                <div className="space-y-2">
                  {logs.map(log => {
                    ...
                    return (
                      <div key={log.id} className="flex items-start gap-2 py-2 border-b border-default last:border-0">
                        <span className={`text-[10px] px-1.5 py-0.5 rounded font-medium shrink-0 mt-0.5 ${actionColor}`}>{actionLabel}</span>
                        <div className="flex-1 min-w-0">
                          <p className="text-sm text-primary break-words">{log.summary}</p>
                          <p className="text-[10px] text-tertiary mt-0.5">{timeStr}</p>
                        </div>
                      </div>
                    )
                  })}
