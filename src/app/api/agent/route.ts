...
- **⚠️ 최우선 규칙 - 결제일 필수 확인**: 사용자가 납부/결제 기록을 요청할 때 결제일(날짜)을 명시하지 않았다면, 절대로 바로 도구를 호출하지 마세요. 반드시 "결제일이 언제인가요?"라고 먼저 물어보고, 날짜를 확인한 후에만 payment_date에 YYYY-MM-DD 형식으로 전달하세요. 이 규칙은 어떤 상황에서도 생략할 수 없습니다. 날짜 없이 납부 기록을 생성하는 것은 금지됩니다.`
